...
hist::hist(const vector<double> & x, const int Nb, const double Xlow, const double Xhigh) {
...
hist::hist(string & filename) {
    ifstream file1;
    file1.open(filename.c_str(), ios::in);
    if (!file1) {
        cerr << "hist unable to open file: " << filename << endl;
        exit(121);
    }
    string line;
    // title
    ...
    int i = split(s, line, " \t");
    if (i < 6) {
        cerr << "hist unable to parse file header: " << filename << endl;
        exit(122);
    }
    ...
    while (getline(file1, line)) {
        ...
        i = split(s, line, " \t");
        if (i < 4) {
            cerr << "hist unable to parse file : " << filename << endl;
            cerr << "line  : " << line << endl;
            exit(123);
        }
        Nbin++;
...
void hist::Initialize(const int Nb, const double Xlow, const double Xhigh) {
    // initialize
    this->Nbin = Nb;
    this->xlow = Xlow;
    this->xhigh = Xhigh;
...
void hist::Fill(const vector<double> & x, const int Nb, const double Xlow, const double Xhigh) {
...
void hist::Fill1(int x1) {
    this->Fill1(double(x1));
}

void hist::Fill1(short x1) {
    this->Fill1(double(x1));
}
...
void hist::FillW(double x1, double w1) {
    // initialize
    if (this->Nbin < 1) {
        cerr << "Histogram not initialized for Fill1" << endl;
    }
...
void hist::FillW(int x1, double w1) {
    this->FillW(double(x1),w1);
}

void hist::FillW(short x1, double w1) {
    this->FillW(double(x1),w1);
}
...
void hist::setTitle(const string & s1) {
    title = s1;
}

void hist::setXlabel(const string & s1) {
    xlabel = s1;
}

void hist::setBinLabels(vector<string> & vs) {
    binlabels = vs;
}
...
C_HistoGroups::C_HistoGroups(string & filename) {
	...
	file1.open(filename.c_str(), ios::in);
	if (!file1) {
		cerr << "hists unable to open file: " << filename << endl;
		exit(124);
	}
	bool more =true;
	while (more){ 
...
hists::hists(ifstream & file1) {
	...
	while (more) {
		...
		i = split(s, line, " \t");
		if (i < 6) {
			cerr << "hist unable to parse file header: " << endl;
			exit(128);
		}
		h1.Ntot = string2Double(s[0]);
		h1.mean = string2Double(s[1]);
...
/*
 ostream & operator<<(ostream &output, const hists & Hs) {
 map<string, hist,less<string> >::iterator ih; // = Hs.h.begin();   
